...
콜백에 람다를 넘기고 싶은데 그게 안되서 한시간 정도 헤매다가 전역함수로 깔고, 상태도 어찌 해야 하나 하다가 전역변수로 전부 깔았다. 그 와중에 `emscripten::val`은 기본 생성자가 `delete`되어 있어서 `null`로 초기화해줘야 된다. 이유는 모르겠으나 구현 마음대로겠지 뭐 ㅋㅋ
