...
```
원래 이럴 때 쓰라고 [BOOST_STRONG_TYPEDEF](http://www.boost.org/doc/libs/release/libs/serialization/doc/strong_typedef.html)가 있는데, 탬플릿 대신 매크로를 사용한 구현이다. 개인적으로 매크로를 별로 좋아하지 않아서 탬플릿을 썼다.

혹시 익숙한 느낌이 드는 사람도 있겠지만, 이는 하스켈에서 `type`과 `newtype`같은 관계이다.
